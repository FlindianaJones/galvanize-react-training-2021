    ...
    return (
        <div className="App">
            ...
            <form onSubmit={handleSearchSubmit}>
                <label>
                    Make:
                    <input data-testid="make-search" name='make' value={make} onChange={e => setMake(e.target.value)} />
                </label>
                <label>
                    Model:
                    <input data-testid="model-search" name='model' value={model} onChange={e => setModel(e.target.value)} />
                </label>
                <label>
                    Year:
                    <input data-testid="year-search" name='year' value={year} onChange={e => setYear(parseInt(e.target.value, 10) || 0)} />
                </label>
                <button type='submit'>Search</button>
            </form>
            {cars
                .filter(car => !searchParams.make || car.make === searchParams.make)
                .filter(car => !searchParams.model || car.model === searchParams.model)
